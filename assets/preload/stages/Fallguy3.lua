...
function onCreate()
	...
	local slot = 2;
	local src = '';
	local width = 2000 * size[slot];
	local scale = 1.8 / size[slot];

	src = path[slot]..'Fondo';		addBackgroundSprite('bg1', src, -1200, -550, scale, 1);
	src = path[slot]..'Objetos1';	addMarqueeSprite('bg2-1', src, -600, -1000, width, -400, 3, scale);
	src = path[slot]..'Objetos2';	addMarqueeSprite('bg2-1', src, -1200, -1000, width, -400, 3, scale);
	...
	src = path[slot]..'Objeto_6';	addMarqueeSprite('bg4-6', src, -1650+400, -900, width, -700, 3, scale);
	src = path[slot]..'Objeto_7';	addMarqueeSprite('bg4-7', src, -950, -1000, width, -700, 3, scale);
	src = path[slot]..'Blood_3';	addMarqueeSprite('bg5', src, -1200, -200, width, -1200, 3, scale);
	src = path[slot]..'Piso';		addMarqueeSprite('bg6', src, -1200, 450, width, -2000, 3, scale);
